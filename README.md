...
| `nuclei` | latest release, templates pre-fetched |
| `reference` | SecLists + rockyou |
| `evilwinrm` | evil-winrm |
| `manspider` | SMB crawler |
| `cli_tools` | Responder, sqlmap, rlwrap, exiftool, freerdp3-x11, ffuf, feroxbuster, snmp |
...
| tmux | `0`-indexed, vi keys, 10k history, `allow-rename off`, `prefix`+`C` pipes a command's output to `xclip` | `--skip-tmux` |
