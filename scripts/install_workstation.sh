...
# --- mouse: stock tmux behaviour -- wheel scrolls/enters copy-mode on its own, no
# custom bindings on top (those were fighting tmux and breaking copy). set-clipboard
# lets tmux's own default copy (drag-release, y) sync out via the terminal (OSC 52).
set -g mouse on
