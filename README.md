...
| Part | What | Skip |
|---|---|---|
| Colors | HTB palette forced on tmux, `ls`, and the terminal profile — 🟢 `#9FEF00` exec/accent, 🔵 `#004CFF` dirs/status, ⬛ `#141A26` backgrounds | `--skip-colors` |
| tmux | `0`-indexed, vi keys, 200k history, `\|` `-` splits, mouse scroll + drag-copy → `xclip` | `--skip-tmux` |
| Firefox | FoxyProxy + uBlock + Wappalyzer → Burp `127.0.0.1:8080`, BloodHound stays on `8088` | `--skip-firefox` |
| VS Code | ILSpy decompiler + Snyk scanner installed, codium removed (code only) | `--skip-code` |
