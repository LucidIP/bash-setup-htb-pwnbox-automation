...
echo "🧹 [4/7] Neo4j / BloodHound (native packages, ~36GB reclaimed)..."
...
echo "🧹 [5/7] Penelope + rlwrap..."
command -v uv >/dev/null 2>&1 && uv tool uninstall penelope-shell-handler 2>/dev/null
sudo apt remove -y rlwrap 2>/dev/null

echo "🧹 [6/7] Wordlists, /opt (wipes pivot/tools/hashcat/bloodhound/seclists dirs), Ghidra..."
sudo rm -rf /usr/share/wordlists/* /opt/* 2>/dev/null
sudo apt remove --purge -y seclists ghidra 2>/dev/null

echo "🧹 [7/7] apt autoremove/autoclean..."
