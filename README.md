...
| `cli_tools` | Responder, sqlmap, rlwrap, exiftool, freerdp3-x11 | apt |
| `workstation` | tmux + Firefox proxy stack + VS Code (ILSpy) | conf |
...
White text, green files, blue dirs. Mouse scroll + drag-copy, `y` or `Enter` — all pipe straight to `xclip` (system clipboard).
...
**Performance** — animations off, compositor kept on (smooth redraws on the streamed desktop), single workspace, deep cache wipe.
