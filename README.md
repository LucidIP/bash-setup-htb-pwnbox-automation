...
| VS Code | ILSpy + Snyk installed, codium and its shortcut removed | `--skip-code` |

🟢 `#9FEF00` files/accent · 🔵 `#004CFF` dirs/status · ⬛ `#141A26` backgrounds

Colors verified: green + navy from [hackthebox.com](https://www.hackthebox.com)'s own brand guide, blue/accents from the established HTB terminal scheme ([audibleblink/hackthebox.vim](https://github.com/audibleblink/hackthebox.vim)). Forced inside tmux only — copy/paste is plain tmux + terminal, no extra config on top.
