#!/bin/bash
set -e
# install_workstation.sh — tmux (pwnbox theme, mouse scroll+copy across full history) + Firefox (FoxyProxy/uBlock/Wappalyzer -> Burp 127.0.0.1:8080) + VS Code (ILSpy .NET decompiler ext).
...
# ILSpy: .NET assembly decompiler extension, skipped silently if VS Code isn't installed
command -v code >/dev/null 2>&1 && code --install-extension icsharpcode.ilspy-vscode --force >/dev/null 2>&1

echo "🔍 workstation"; tmux -V; echo "term=$TERM_DEF"
command -v code >/dev/null 2>&1 && echo "vscode-ext=ilspy-vscode"
echo "✅ tmux + firefox + vscode ready (running tmux: tmux kill-server for a full reload)"
